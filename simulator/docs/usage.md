...
> ✔ **行尾 `CR` / `LF` / `CRLF` 都行，而且会逐字回显**（2026-09-21 加固后的固件）。
> 旧版（`text 14818` 及以前）**只认 LF 且不回显**：只会发 CR 的终端（PuTTY 默认）会“敲 `AT` 没反应”，
> 而且残留字符会粘住下一条命令（见 §7）；用旧版时把 PuTTY 勾上 `Implicit LF in every CR`。
