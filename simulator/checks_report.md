...
| 套件 | 结果 | 用时 |
|---|---|---|
| 模拟器回归（AT/连接/转发/配对/串口空口/host 数据口） | 通过 | 39.70s |
...
```
PASS  字典 zh/en key 集合一致（168 / 168）
PASS  每个 key 恰好 2 次（zh + en）
PASS  扫描 2 个页面/脚本，引用 160 个 key（动态前缀家族 5 个）
PASS  页面引用的 key 全部在字典里

文案字典：全部通过（168 个 key，引用 160 个）
```
