...
| 区域 | 功能 |
|------|------|
| **拓扑（设备信息唯一来源）** | 设备 A / B 卡片：设备名 + **类型**（CH32V203 虚拟机/真机、T-Halow-RJ45 虚拟机/真机）+ **模式**（AP/STA）+ **端口**（TCP :9001 / 串口 COM5 / COM3）+ SSID + 频率/带宽 + **发射功率（开了距离模型时显示 `@ Nm`）** + **关联 STA 数**（AP）与 **STA 列表**（MAC + 实时 RSSI）+ 连接状态 + RSSI 信号条 + TX/RX/运行时长；中间为链路状态与流动动画（TCP 虚拟空口 / 串口空口 / UART2 物理空口） |
...
| **配置面板** | 表单生成 AT 命令（模式/SSID/加密/PSK/带宽/信道/**发射功率**），一键下发到指定设备；**距离(m) / 损耗指数**是模拟器扩展（真机无此命令）且是**链路属性** → 会同时下发给 A/B 两台模拟器 |
