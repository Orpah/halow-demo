...
(function (global) {
  ...
  var D = {
    zh: {
      ...
      "at:def:AT+ASSOC_FAIL=": "[%] 关联失败概率（模拟器扩展）",
      ...
      "at:def:AT+DIST=": "[米] 距离模型：>0 开启（RSSI = 对端发射功率 - 路径损耗）",
      ...
      "at:def:AT+TWT=": "[间隔ms[,窗口ms]] TWT 唤醒窗口，STA 侧（模拟器扩展）",
      ...
      "tui_cfg_mode": "模式",
      "tui_cfg_pl": "损耗指数",
      "tui_cfg_pl_hint": "模拟器扩展（真机无此命令）",
      "tui_cfg_psk": "PSK(64hex)",
      "tui_cfg_psk_ph": "WPA-PSK 时填写",
      "tui_cfg_raw": "RAW 槽位",
      "tui_cfg_title": "配置面板",
      "tui_cfg_twt": "TWT 间隔(ms)",
      "tui_cfg_simrow": "丢包/关联失败下发给两台；RAW 只对 AP 有意义、TWT 只对 STA 有意义 → 这两项下发给上面“设备”里选的那台（下发前先把模式设对）",
      "tui_cfg_tx": "发射功率(dBm)",
      "tui_clear_btn": "清屏",
    ...
    en: {
      ...
      "at:def:AT+ASSOC_FAIL=": "[%] association failure probability (simulator extension)",
      ...
      "at:def:AT+DIST=": "[m] distance model: >0 enables (RSSI = peer TX power - path loss)",
      ...
      "at:def:AT+LOSS=": "[%] data-frame loss injection (simulator extension)",
      ...
      "at:def:AT+PATHLOSS=": "[n] path-loss exponent (2 = free space, 2.7-4 indoor)",
      "at:def:AT+PSK=": "[64 hex] passphrase",
      "at:def:AT+PS_MODE=": "[0..4] STA sleep mode",
      "at:def:AT+RAW=": "[slots] RAW access window, AP side only (simulator extension)",
      "at:def:AT+ROAM=": "[0/1] roaming switch (STA side)",
      "at:def:AT+RSSI": "[?/index/MAC] query RSSI",
      "at:def:AT+STALIST": "AP associated-STA table (MAC=RSSI, simulator extension)",
      ...
      "at:def:AT+TWT=": "[interval_ms[,window_ms]] TWT wake window, STA side (simulator extension)",
      ...
      "tui_cfg_afail": "Assoc fail (%)",
      "tui_cfg_bw": "BW",
      "tui_cfg_chan": "Channel",
      "tui_cfg_dev": "Device",
      "tui_cfg_dist": "Distance (m)",
      "tui_cfg_enc": "Encryption",
      "tui_cfg_hint": "(generates AT commands)",
      "tui_cfg_loss": "Loss (%)",
      "tui_cfg_mode": "Mode",
      "tui_cfg_pl": "Path loss n",
      "tui_cfg_pl_hint": "Simulator extension (absent on real boards)",
      "tui_cfg_psk": "PSK(64hex)",
      "tui_cfg_psk_ph": "fill in for WPA-PSK",
      "tui_cfg_raw": "RAW slots",
      "tui_cfg_title": "Config",
      "tui_cfg_twt": "TWT interval (ms)",
      "tui_cfg_simrow": "Loss / assoc-fail go to both devices; RAW matters only on an AP and TWT only on a STA -> these two go to the device selected above (set its mode first)",
      "tui_cfg_tx": "TX power (dBm)",
      "tui_clear_btn": "Clear",
