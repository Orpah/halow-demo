...
## 0. ORPAH 业务已迁出本仓库（2026-09-12）

ORPAH-over-HaLow 的业务全链路（Router 桥 / 走失表 / 报文集 / ID 验签 / 定位 / 告警 / IoTDB / Web UI，
原 `simulator/orpah/`）已整体迁到**独立仓库**：
`F:\git\orpah-over-halow`（GitHub: langhua/orpah-over-halow，`git subtree split` 保留历史）。
...
- **两个仓库怎么相接**：业务侧通过 **host 数据口（TCP）** 与本仓库的空口仿真对接；
  ...
  不 import `sim.py`），换真实 SPI 时只替换底层收发。
- **ORPAH 业务规则 / 踩坑**：见 `orpah-over-halow/AGENTS.md`（原 §0 全部内容已搬过去）。
- **本项目自持一份文案字典**：`tools/ui/static/ui_i18n.js`（2026-09-12 从"与 orpah 共用的一份"拆出，
  只留本 UI 用到的 158 个 key）。改文案时若发现 key 缺失，用 `node --check` 与页面实测核对；
  拆分脚本与归属判据见提交历史（`halow-demo` 2026-09-12 那次）。
...
- **只提交「我自己创建的文件」**：我写的源码/文档/脚本 = **我们的成果（有版权）**，可以入库；
  **不是我创建的一律不提交** —— 别人的手册 PDF、厂商 SDK 源码、固件 bin、安装包、下载的第三方素材
  **都没有版权**，入库即无授权传播。
- 提交前先 `git rev-parse --show-toplevel` 确认仓库；**禁止 `git add -A`**，一律显式写路径
  （如 `git add simulator/host/sim.py`）；提交后 `git show --stat` 核一遍清单。
- 反面教训（2026-09-16）：终端把命令开头的 `Set-Location` 丢掉 → `git add -A` 落在 `F:\git\tianlu` →
  19 个厂商 PDF + 整个 SDK 投放物（6937 文件 / 494 万行）被提交，提交信息却只写了「docs §7.5」；
  当晚我核查的是**错的仓库**（只查本仓）还误报「没有发生」。2026-09-18 已撤销（`reset --mixed` + 清对象）。
...
- **图要与文字规格一致**：图的权威内容 = **可复验的源**（例：Fritzing `.fzz` 的网表，用
  `simulator/tools/fzz_nets.py` 逐网核对）；手工加在导出 `.svg` 上的标注要写明「手加、重导出会丢」。
- 本仓现成素材：`simulator/hardware/wiring/*.svg`（接线图）、`images/*.png`（模块照片）。
  **这类误提交只在 push 前能救。**
